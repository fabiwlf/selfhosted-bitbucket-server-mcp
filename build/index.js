...
export class BitbucketServer {
    ...
    setupToolHandlers() {
        ...
        this.server.setRequestHandler(ListToolsRequestSchema, async () => ({
            tools: [
                ...
                {
                    name: 'mark_pull_request_as_draft',
                    description: 'Mark a pull request as draft. Use this to indicate that the pull request is not yet ready for review or merging.',
                    inputSchema: {
                        type: 'object',
                        properties: {
                            project: { type: 'string', description: 'Bitbucket project key. If omitted, uses BITBUCKET_DEFAULT_PROJECT environment variable.' },
                            repository: { type: 'string', description: 'Repository slug containing the pull request.' },
                            prId: { type: 'number', description: 'Pull request ID to mark as draft.' }
                        },
                        required: ['repository', 'prId']
                    }
                },
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            try {
                ...
                if (this.config.readOnly && !readOnlyTools.includes(request.params.name)) {
                    throw new McpError(ErrorCode.MethodNotFound, `Tool ${request.params.name} is not available in read-only mode`);
                }
                // Helper function to get project with fallback to default
                const getProject = (providedProject) => {
                ...
                switch (request.params.name) {
                    ...
                    case 'decline_pull_request': {
                        const declinePrParams = {
                            project: getProject(args.project),
                            repository: args.repository,
                            prId: args.prId
                        };
                        return await this.declinePullRequest(declinePrParams, args.message);
                    }
                    case 'add_comment': {
                        const commentPrParams = {
                            project: getProject(args.project),
                            repository: args.repository,
                            prId: args.prId
                        };
                    ...
                    case 'get_diff': {
                        ...
                        return await this.getDiff(diffPrParams, args.contextLines, args.maxLinesPerFile);
                    }
                    case 'get_reviews': {
                        const reviewsPrParams = {
                            project: getProject(args.project),
                            repository: args.repository,
                            prId: args.prId
                        };
                    ...
                    case 'delete_branch': {
                        return await this.deleteBranch(getProject(args.project), args.repository, args.branch);
                    }
                    case 'mark_pull_request_as_draft': {
                        const draftPrParams = {
                            project: getProject(args.project),
                            repository: args.repository,
                            prId: args.prId
                        };
                        return await this.markPullRequestAsDraft(draftPrParams);
                    }
                    case 'approve_pull_request': {
                        const approvePrParams = {
                            project: getProject(args.project),
                            repository: args.repository,
                            prId: args.prId
                        };
    ...
    async createPullRequest(input) {
        ...
        if (input.includeDefaultReviewers !== false) {
            try {
                const targetRepoResponse = await this.api.get(`/projects/${input.project}/repos/${input.repository}`);
                ...
                const defaultReviewersResponse = await this.api.get(`/projects/${input.project}/repos/${input.repository}/reviewers`, {
    ...
    async updatePullRequest(input) {
        ...
        if (!project || !repository || !prId) {
            throw new McpError(ErrorCode.InvalidParams, 'Project, repository, and prId are required');
        }
        const current = await this.api.get(`/projects/${project}/repos/${repository}/pull-requests/${prId}`);
        const updated = {
            ...current.data,
        ...
        const response = await this.api.put(`/projects/${project}/repos/${repository}/pull-requests/${prId}`, updated);
        return {
            content: [{ type: 'text', text: JSON.stringify(response.data, null, 2) }]
        };
    }
    ...
    async mergePullRequest(params, options = {}) {
        ...
        const prResponse = await this.api.get(`/projects/${project}/repos/${repository}/pull-requests/${prId}`);
    ...
    async addCommentInline(params, options) {
        const { project, repository, prId } = params;
        if (!project || !repository || !prId || !options.filePath || !options.line || !options.lineType) {
            throw new McpError(ErrorCode.InvalidParams, 'Project, repository, prId, filePath, line, and lineType are required');
        }
        const { text, parentId, state, severity } = options;
        const response = await this.api.post(`/projects/${project}/repos/${repository}/pull-requests/${prId}/comments`, {
            text,
    ...
    async editComment(params, options) {
        ...
        const response = await this.api.put(`/projects/${project}/repos/${repository}/pull-requests/${prId}/comments/${options.commentId}`, {
            text: options.text,
    ...
    async deleteComment(params, options) {
        ...
        await this.api.delete(`/projects/${project}/repos/${repository}/pull-requests/${prId}/comments/${options.commentId}`, { params: { version: options.version } });
        return {
            content: [{ type: 'text', text: JSON.stringify({ deleted: true, commentId: options.commentId }, null, 2) }]
        };
    }
    async publishReview(params, options) {
        const { project, repository, prId } = params;
        if (!project || !repository || !prId) {
            throw new McpError(ErrorCode.InvalidParams, 'Project, repository, and prId are required');
        }
        const response = await this.api.put(`/projects/${project}/repos/${repository}/pull-requests/${prId}/review`, {
    ...
    async getCodeInsights(params) {
        const { project, repository, prId } = params;
        if (!project || !repository || !prId) {
            throw new McpError(ErrorCode.InvalidParams, 'Project, repository, and prId are required');
        }
        const reportsResponse = await this.api.get(`/projects/${project}/repos/${repository}/pull-requests/${prId}/reports`, { baseURL: `${this.config.baseUrl}/rest/insights/latest` });
        ...
        for (const report of reports) {
            ...
            try {
                const annotationsResponse = await this.api.get(`/projects/${project}/repos/${repository}/pull-requests/${prId}/reports/${reportKey}/annotations`, { baseURL: `${this.config.baseUrl}/rest/insights/latest` });
    ...
    async getDashboardPullRequests(options = {}) {
        ...
        if (participantStatus)
            params.participantStatus = participantStatus;
        if (order)
            params.order = order;
        if (closedSince)
            params.closedSince = closedSince;
        const response = await this.api.get('/dashboard/pull-requests', { params });
        return {
            content: [{ type: 'text', text: JSON.stringify(response.data, null, 2) }]
        };
    }
    truncateDiff(diffContent, maxLinesPerFile) {
    ...
    truncateFileSection(fileLines, fileName, maxLines) {
    ...
    async getDiff(params, contextLines = 10, maxLinesPerFile) {
        const { project, repository, prId } = params;
        if (!project || !repository || !prId) {
            throw new McpError(ErrorCode.InvalidParams, 'Project, repository, and prId are required');
        }
    ...
    async getComments(params) {
        ...
        const response = await this.api.get(`/projects/${project}/repos/${repository}/pull-requests/${prId}/activities`);
        const comments = response.data.values.filter((activity) => activity.action === 'COMMENTED');
        return {
    ...
    async search(options) {
        ...
        if (type === 'file') {
            ...
            if (!query.includes('ext:') && !query.startsWith('"')) {
                ...
                if (repository && project)
                    searchQuery += ` repo:${project}/${repository}`;
            }
        }
        ...
        try {
            ...
            const searchResults = {
                ...
                results: codeResults.values?.map((result) => ({
    ...
    async getFileContent(options) {
        ...
        if (!project || !repository || !filePath) {
            throw new McpError(ErrorCode.InvalidParams, 'Project, repository, and filePath are required');
        }
        const params = {
        ...
        const response = await this.api.get(`/projects/${project}/repos/${repository}/browse/${filePath}`, { params });
        const fileContent = {
            project,
            repository,
            filePath,
            ...
            startLine: start,
            lines: response.data.lines?.map((line) => line.text) || []
        };
        return {
            content: [{ type: 'text', text: JSON.stringify(fileContent, null, 2) }]
        };
    }
    async browseRepository(options) {
        const { project, repository, path = '', branch, limit = 50 } = options;
        if (!project || !repository) {
            throw new McpError(ErrorCode.InvalidParams, 'Project and repository are required');
        }
        const params = {
            limit
        };
        ...
        const response = await this.api.get(`/projects/${project}/repos/${repository}/browse${browsePath}`, { params });
    ...
    async listPullRequests(options) {
        ...
        const response = await this.api.get(`/projects/${project}/repos/${repository}/pull-requests`, { params });
    ...
    async listCommits(options) {
        ...
        const response = await this.api.get(`/projects/${project}/repos/${repository}/commits`, { params });
    ...
    async deleteBranch(project, repository, branch) {
        if (!project || !repository || !branch) {
            throw new McpError(ErrorCode.InvalidParams, 'Project, repository, and branch are required');
        }
        // Pre-check: prevent deletion of default branch
        try {
            const defaultBranchResponse = await this.api.get(`/projects/${project}/repos/${repository}/default-branch`);
            if (defaultBranchResponse.data?.displayId === branch) {
                throw new McpError(ErrorCode.InvalidParams, `Cannot delete the default branch "${branch}". Change the default branch first.`);
            }
        }
        catch (error) {
    ...
    async markPullRequestAsDraft(params) {
        const { project, repository, prId } = params;
        if (!project || !repository || !prId) {
            throw new McpError(ErrorCode.InvalidParams, 'Project, repository, and prId are required');
        }
        const url = `${this.config.baseUrl}/rest/ui/latest/projects/${project}/repos/${repository}/pull-requests/${prId}/draft`;
        const response = await axios.put(url, { draft: true }, {
            headers: this.config.token
                ? {
                    Authorization: `Bearer ${this.config.token}`,
                    'Content-Type': 'application/json'
    ...
    async approvePullRequest(params) {
        ...
        const response = await this.api.post(`/projects/${project}/repos/${repository}/pull-requests/${prId}/approve`, {}, { headers: { 'Content-Type': 'application/json' } });
        return {
            content: [{ type: 'text', text: JSON.stringify(response.data, null, 2) }]
        };
    }
    async unapprovePullRequest(params) {
        const { project, repository, prId } = params;
        if (!project || !repository || !prId) {
            throw new McpError(ErrorCode.InvalidParams, 'Project, repository, and prId are required');
        }
        const response = await this.api.delete(`/projects/${project}/repos/${repository}/pull-requests/${prId}/approve`, { headers: { 'Content-Type': 'application/json' } });
        return {
            content: [{ type: 'text', text: JSON.stringify(response.data, null, 2) }]
        };
    }
